...
/*
char* getOffset(SDL_Surface *src, int x, int y) {
...
void SDL_BlitSurface(SDL_Surface *src, SDL_Rect *srcrect, 
	...
	for(int i = 0; i < h; i++) {
		char* dst_addr = (char*)dst->pixels + (dy + i)*dst->w + dx;
		char* src_addr = (char*)src->pixels + (sy + i)*src->w + sx;
...
void SDL_FillRect(SDL_Surface *dst, SDL_Rect *dstrect, uint32_t color) {
	...
	for(int h = 0; h < dst_h; h++) 
		memset(dst->pixels + x + (y + h)*dstrect->w, color, w);
