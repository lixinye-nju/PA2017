...
uint32_t internal_float_add(uint32_t b, uint32_t a) {
	...
	//printf("fa.fval:%f\tfb.fval:%f\tfa.exp:%x\tfb.exp:%x\tfa.frac:%x\tfb.frac:%x\n", fa.fval, fb.fval, fa.exponent, fb.exponent, fa.fraction, fb.fraction);
	...
	assert(0);
	assert(shift >= 0);*/
	if((fa.exponent != 0)||(fa.exponent == 0 && fb.exponent == 0))
...
uint32_t internal_float_mul(uint32_t b, uint32_t a) {
	...
	exp_res = fa.exponent + fb.exponent - 23 - 127 + 3;
