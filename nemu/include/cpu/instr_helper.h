...
#define make_instr_impl_r(inst_name, reg, suffix) \
...
#define make_instr_impl_m(inst_name, reg, suffix) \
	make_instr_func(concat5(inst_name, _, reg, _, suffix)) {\
		int len = 1; \
		concat(decode_data_size_, suffix); \
		...
		opr_dest.addr = reg; \
		instr_execute_1op(); \
		print_asm_1(""#inst_name"	", "", len, &opr_dest); \
		return len; \
	}
...
#define condition_na \
	cpu.eflags.ZF == 1 || cpu.eflags.ZF == 1
